...
const Sidebar = ({
    ...
    return (
        <aside
            id="logo-sidebar"
            className={"fixed top-0 left-0 z-10 w-64 h-screen pt-14 overflow-auto transition-transform bg-white border-r border-gray-200  dark:bg-gray-800 dark:border-gray-700" + (sidebar ? ' sm:translate-x-0 shadow-xl dark:shadow-gray-900' : " -translate-x-full")}
            aria-label="Sidebar"
        >
            {
                menus.length > 0 &&
                            ...
                            menus?.slice().sort((a, b) => a.orden - b.orden)?.map((menu, index) => (
                                <li key={index}>
                                    <Link
                                        ...
                                        className={`flex items-center p-2 rounded-lg dark:text-gray-200 font-semibold hover:bg-yellow-400 dark:hover:bg-gray-800 justify-end ${params['*']?.includes(`menu/${menu?.nombre}`) ? 'dark:bg-gray-800 bg-yellow-500 dark:text-yellow-500 text-gray-800' : ''}`}
                                        onClick={clicMenu}
                                    >
                                        <div>
